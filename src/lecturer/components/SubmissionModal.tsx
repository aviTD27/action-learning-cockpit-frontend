...
export default function SubmissionModal({ open, existing, onClose, onSave }: Props) {
  ...
  const activeCourses = courses.filter(
    c => c.status === 'ACTIVE' || c.id === existing?.courseId,
  )
  ...
  useEffect(() => {
    if (open) {
      ...
      setCourseIds(existing?.courseId ? [existing.courseId] : [])
  ...
  const buildData = (status: 'DRAFT' | 'PUBLISHED' | undefined): CreateSubmissionData | null => {
    ...
    const selected = isEditing ? (courseId === '' ? [] : [courseId as number]) : courseIds
    if (selected.length === 0) { setError('Select at least one course'); return null }
    ...
    const firstCourse = activeCourses.find(c => c.id === selected[0])
    return {
      title: title.trim(),
      ...
      courseIds: isEditing ? undefined : selected,
      dueDate,
          ...
          <div className="ua-form-grid">
            {isEditing ? (
              <div className="ua-modal-field full">
                ...
                <select className="ua-modal-input" value={courseId}
